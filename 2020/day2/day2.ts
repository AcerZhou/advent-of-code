...
for (let i = 0; i < day2Input.length; i++) {
    const str: any = day2Input[i][3];
    const c: any = day2Input[i][2];

    let count = (str.match(new RegExp(c, "g")) || []).length;

    if (count >= day2Input[i][0] && count <= day2Input[i][1]) {
        n++;
    }
}
...
for (let i = 0; i < day2Input.length; i++) {
    const first: number = +day2Input[i][0] - 1;
    const second: number = +day2Input[i][1] - 1;
